...
- Before Entire stores any copy of a transcript it applies the agent's optional `agent.TranscriptSanitizer` (`agent.SanitizeTranscriptForStorage`), which strips non-portable agent state — Codex's encrypted reasoning payloads and compaction blobs, which are bound to the originating session and cannot be replayed out of a checkpoint. This runs at the point Entire takes custody (the Stop path in `lifecycle.go`, before `.entire/metadata/<session>/full.jsonl` is written) and therefore **before** redaction, which is required in both directions: it keeps non-replayable payloads out of storage, and it stops the redaction layers from scanning ciphertext they would only discard (base64 is the pathological input for the entropy layer, so a large Codex rollout otherwise costs tens of seconds per Stop). The transform is idempotent, so downstream write paths call it without needing to know whether an upstream path already did. The agent's own transcript is never modified. Coupling to respect: `SessionState.CheckpointTranscriptSize` is a growth baseline compared against the shadow transcript blob size in `sessionHasNewContent`, so it must be measured in the same sanitized coordinate — that is `CondenseResult.TranscriptSizeBaseline`; using the raw size makes the comparison false forever and the session silently stops condensing.
