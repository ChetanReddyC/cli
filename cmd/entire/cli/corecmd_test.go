...
// filterSortRow is the test fixture: two columns, REPO and PRIVATE, mapped by
// filterSortRowFn — exercises sortRows over a real multi-column row.
type filterSortRow struct {
	repo    string
	private string
}

var filterSortHeaders = []string{"REPO", "PRIVATE"}

func filterSortRowFn(r filterSortRow) []string { return []string{r.repo, r.private} }

func filterSortRepos(items []filterSortRow) []string {
...
func TestSortRows(t *testing.T) {
	...
	t.Run("empty spec sorts by first column ascending", func(t *testing.T) {
		t.Parallel()
		items := []filterSortRow{{repo: "b/y"}, {repo: "a/x"}, {repo: "c/z"}}
		if err := sortRows(items, filterSortHeaders, filterSortRowFn, ""); err != nil {
			t.Fatalf("sortRows: %v", err)
		}
		if want := []string{"a/x", "b/y", "c/z"}; !equalStrings(filterSortRepos(items), want) {
			t.Errorf("default sort: got %v, want %v", filterSortRepos(items), want)
		}
	})
	...
	t.Run("leading dash sorts descending", func(t *testing.T) {
		t.Parallel()
		items := []filterSortRow{{repo: "b/y"}, {repo: "a/x"}, {repo: "c/z"}}
		if err := sortRows(items, filterSortHeaders, filterSortRowFn, "-repo"); err != nil {
			t.Fatalf("sortRows: %v", err)
		}
		if want := []string{"c/z", "b/y", "a/x"}; !equalStrings(filterSortRepos(items), want) {
			t.Errorf("descending sort: got %v, want %v", filterSortRepos(items), want)
		}
	})
	...
	t.Run("sorts by named column", func(t *testing.T) {
		...
		if err := sortRows(items, filterSortHeaders, filterSortRowFn, "private"); err != nil {
			t.Fatalf("sortRows: %v", err)
		}
		// "no" < "yes", so the second row comes first.
		if want := []string{"b", "a"}; !equalStrings(filterSortRepos(items), want) {
			t.Errorf("named-column sort: got %v, want %v", filterSortRepos(items), want)
		}
	})
	...
	t.Run("stable for equal keys", func(t *testing.T) {
		...
		if items[0].private != "1" || items[1].private != "2" || items[2].private != "3" {
			t.Errorf("stability: equal keys reordered: %+v", items)
		}
	})

	t.Run("unknown column errors naming valid columns", func(t *testing.T) {
		t.Parallel()
		items := []filterSortRow{{repo: "a"}}
		err := sortRows(items, filterSortHeaders, filterSortRowFn, "nope")
		if err == nil {
			t.Fatal("unknown column: expected error, got nil")
		}
		if !strings.Contains(err.Error(), "unknown sort column") || !strings.Contains(err.Error(), "repo") {
			t.Errorf("unknown column: message %q must name the bad column and valid columns", err)
		}
	})
}
