...
3. Explain the top one or two hits with ` + "`entire checkpoint explain <id>`" + ` (checkpoint ID or commit SHA). For a checkpoint hit from another repo, add ` + "`--repo <owner/name>`" + ` — it needs the full checkpoint ID from the compact hit. For a session hit on the current branch, bridge with ` + "`entire checkpoint explain --session <id>`" + ` — it lists that session's checkpoints; explain one of those.
4. Only if the scoped detail is not enough, add ` + "`--full`" + ` to pull the checkpoint's entire session transcript. For repo, pr, other-repo commit, and other-branch session hits, summarize from the compact fields alone; ` + "`explain`" + ` cannot read them.
