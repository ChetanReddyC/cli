...
| *(no interface)* `Event.SubagentTranscriptPath` | — | Set it on a `SubagentEnd` event when the agent's hook payload names the subagent's own transcript (Codex `agent_transcript_path`, Cursor the same). Without it the framework falls back to guessing Claude Code's nested layout, which silently resolves to nothing for every other agent — so the task checkpoint stores no subagent transcript and file extraction reads the main transcript, where a subagent's edits never appear. |
