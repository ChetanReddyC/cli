...
**Where enumeration looks:** with a dedicated `checkpoint_remote` configured it queries the resolved checkpoint URL (`remote.FetchURL`, which can fall through to origin in edge cases) — unchanged single-target behavior. With **none** configured it ls-remotes **every checkpoint read candidate** (the elected sync remote, then the legacy `origin` tier) and **merges the listings** — a union deduped by ref name — so a second machine discovers refs-native checkpoints in the default setup without any dedicated store. Merging, not first-non-empty, is deliberate: before single-remote sync, per-checkpoint refs landed on whichever remote the pre-push hook fired for, so *disjoint* legacy refs on origin coexisting with new refs on the elected remote are realistic, and first-non-empty would shadow one side. One candidate failing logs at debug and does not block the others; when every candidate fails, the lister returns nothing and `List` stays local-only (the read paths' "surface the first candidate's error" rule applies to data fetches, not to this best-effort discovery). Discovery is **best-effort and additive**: an `ls-remote` failure or a short timeout (a few seconds — spanning the whole union; this path must not turn a previously-local listing into a long hang when a remote is unreachable) logs, warns on stderr, and returns the local results rather than failing the whole listing. URL resolution and `ls-remote` run from the worktree root so repo-local git config applies.
